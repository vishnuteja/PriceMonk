...
my $xp = XML::XPath->new(filename => 'eBay_media.xml'); # reading XML file

$query3=qq{update sellers SET seller = ?,price = ?,SWeight = ?,rank = ?,SPrice = ?,url = ?,SDays = ? where productID = ?};
...
foreach my $row ($xp->findnodes('/allmedia/media')) 
{
	...
	if ($sth1->rows < 0) {
		...
		if($product_id ne '')
		{
			$sth3->execute($seller, $Price, $shipwt, $rank, $shipPrice, $url, $shipDay, $product_id) || die "Error in Insertion to Sellers";
		}
		$sth1->finish();
